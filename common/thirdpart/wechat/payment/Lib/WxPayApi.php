...
use common\thirdpart\wechat\payment\Lib\WxPayResults;
...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 30)
	{
        ...
        */
		$inputObj->SetAppid($apiConfig['appid']);//公众账号ID
		$inputObj->SetMch_id($apiConfig['mchid']);//商户号
	...
	public static function refund($inputObj, $timeOut = 30)
	{
		...
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
			...
			throw new WxPayException("退款申请接口中，缺少必填$apiConfig");
		}
		$inputObj->SetAppid($apiConfig['appid']);//公众账号ID
		$inputObj->SetMch_id($apiConfig['mchid']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $apiConfig['sslcert_path']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $apiConfig['sslkey_path']);
		}
		//post提交方式
