...
<div id="webwrapper">
        ...
        <?php if($inputValue):?>
          <!--编辑的时候-->
            <ul class="filelist editfilelist" style="margin-left:10px";>
                <?php
                $list = json_decode($inputValue, true);
                ?>
                <?php if($list):?>
                    ...
                    <li id="imgpath_0" class="state-complete">
                        <p class="title"><?=$k;?></p>
                        <p class="imgWrap">
                            <img src="<?=$config["public"].'/attachment'.$v;?>"></p>
                        <span class="success"></span>
                        <div class="file-panel" style="height: 30px;">
                            <span class="cancel" data-del="<?=$k;?>" data-url="../../public/attachment<?=$v;?>">删除</span></div>
                    </li>
                    <?php endforeach;?>
                <?php endif;?>
    ...
    <div id="filenames" style="display: none">
        <?php if($inputValue):?>
            <?php if($list):?>
                <?php foreach($list as $k=>$v):?>
                <input type="hidden" name="filename[<?=$k;?>]" data-file="<?=$k;?>" value="<?=$v;?>">
                <?php endforeach;?>
            <?php endif;?>
        <?php endif;?>
    </div>
