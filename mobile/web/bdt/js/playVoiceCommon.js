    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.result == "success") {
                // $('#'+currPayObjId+'>em').text("免费收听");
                // alert(currPayObjId);
                //playAudioClickFunction(currPlayId,currPayType,currPayListenType,currPayObjId,currPayAllowPauseBool);
            } else {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.result == "success") {
                    ...
                    alert(7);
                    var iosMoney = parseInt(result.data.toPayInfo.iosMoney);
                    var iosMoneyPrice = parseInt(result.data.toPayInfo.iosMoneyPrice);
                    // alert("iosMoney=" + iosMoney + ",iosMoneyPrice=" + iosMoneyPrice);
                    if (iosMoney < iosMoneyPrice) {
                        popupPay({
                            title: '问房币不足,请充值',
                            useSum: '收听需支付 ' + iosMoneyPrice + ' 问房币',
                            surplus: iosMoney,
                            affirmPay: function (closePopup) {
                                closePopup();
                                cordova.exec(callAppsSuccessFunction, callAppsFailFunction, "MyPlugin", "applePay", [0]);
                            }
                        }, 0);
                    } else {
                        popupPay({
                            ...
                            affirmPay: function (closePopup) {
                                iosCurrency = iosMoney - iosMoneyPrice;
                                closePopup();
                                playAudioClickFunctionEx(id, type, listenType, objId, allowPauseBool, shareId, srParentId, "iosMoneyToPay", playCallBack);
                            }
                        }, 1);
                    }
                } else if (result.result == "toPay") {
                    if(result.data.toPayInfo.payType == 3){
                    ...
                    if (!isUndefined(result.gotoUrl) && result.gotoUrl != null && result.gotoUrl == "login") {
                        getSessionUser();
                    }
                }
            }
