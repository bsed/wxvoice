	...
	getDate: function(){
        $.ajax({
            ...
            success: function(result) {
                var data = result.data.areaList;
                for(var i = 0; i < data.length; i++){
                    if(data[i].pid == 0){
                        urbanSelectionControl.iosProvinces.push({'id': data[i].id.toString(), 'value': data[i].name, 'parentId': data[i].pid.toString()})
                    }else{
                        if(data[i].pid.toString() == urbanSelectionControl.iosProvinces[urbanSelectionControl.iosProvinces.length - 1].id){
                            urbanSelectionControl.iosCitys.push({'id': data[i].id.toString(), 'value': data[i].name, 'parentId': data[i].pid.toString()})
                        }else{
                            urbanSelectionControl.iosCountys.push({'id': data[i].id.toString(), 'value': data[i].name, 'parentId': data[i].pid.toString()})
                        }
                    }
                }
                urbanSelectionControl.start = true;
            }
        });
	},
	popupUrbanSelection: function(parameter,fn){
        var iosSelect = null;
        iosSelect = new IosSelect(3,
            [urbanSelectionControl.iosProvinces, urbanSelectionControl.iosCitys, urbanSelectionControl.iosCountys],
            {
                title: parameter.title?parameter.title:'选择地区',
                itemHeight: parameter.itemHeight?parameter.itemHeight: 35,
                relation: [1, 1, 0, 0],
                oneLevelId: parameter.oneLevelId,
                twoLevelId: parameter.twoLevelId,
                threeLevelId: parameter.threeLevelId,
                callback: function (selectOneObj, selectTwoObj, selectThreeObj) {
                	fn(selectOneObj, selectTwoObj, selectThreeObj);
                }
            }
		);
	}
