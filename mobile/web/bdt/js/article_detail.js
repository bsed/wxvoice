...
function commentList(data){
 ...
 for(var i=0;i<data.list.length;i++){
	...
	list +='<div class="comment-item bc-grey" id="commentList'+data.list[i].id+'"><div class="comment-item-author">' +
		'<a onclick="gotoUser_pageHtml('+data.list[i].id+')">' +
		'<i><img src="'+data.file+data.list[i].user.photo+'"></i><span class="ml5"><i class="fs30 fc-navy">'+data.list[i].user.nickname+'</i>' +
...
function configArticleDetailPart(groups){
	...
	var authorFocusTimeReadStr =	'<div class="author-focus fs24">'+
										'<a class="mr5" onclick="gotoUser_pageHtml('+groups.author.id+')">'+
											'<img src="'+insertImgType(groups.author.headPic,1)+'">'+
											levelStr+
										'</a>'+
										'<div>'+
											'<a class="fc-navy" onclick="gotoUser_pageHtml('+groups.author.id+')">'+groups.author.nickname+'</a>'+
											'<p>'+
...
function configwxShare(result){
	...
	if(result.data.article.content==""||result.data.article.content==null){
		shareFriendStr = "畅聊生活中的买房、收房、装修、维权、业委会方方面面进行讨论、交流和分享";
	}else{
    ...
    //wxShare(shareTitle,shareStr,shareFriendStr,"","");
}
