                ...
                if (isVoiceAns == 0) {
                    ...
                    if (reAnswerStr == 2 || reAnswerStr == 3) {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            dataType: 'json',
                            data: {"qaId":qaId,"deviceType":2,"url":"","answerLen":"","content":$("#textField").val(),"pics":pics,"isAllowForward":isAllowForward},
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            dataType: 'json',
                            data: {"qaId":qaId,"deviceType":2,"url":"","isZhuiwen":isReAns,"answerLen":"","content":$("#textField").val(),"pics":pics,"isAllowForward":isAllowForward},
                            success: function(result){
                                if (result.result=="success") {
                                    dataLoadedSuccess("提交成功!");
                                    window.location.replace("qanda_detail.html?id="+qaId+"&typeId="+typeId);
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.result == "success") {
                        ...
                        /**
                        if (typeId ==1) {
                            window.location.href = "qanda_detail.html?id="+qaId+"&typeId="+typeId;
                        };*/
                        window.location.replace("qanda_detail.html?id="+qaId+"&typeId="+typeId);
                    }else{
                         dataLoadedError(result.message);
...
function postAnswerOfQuestionRequest(wxRecordId,curCount) {
    ...
    $.ajax({
        ...
        success: function(result) {
            clearToastDialog();
            if (result.result == "success") {
                // $("#scanMe a").attr("href","qanda_detail.html?id="+qaId+"&typeId="+typeId)
                // $(".js_dialog").show();
                // $("#scanMe").show();
            ...
            if (result.result == "success") {
                ...
                wx.ready(function() {
                  clearToastDialog();
                    ...
                    // wx.onVoicePlayEnd({
                    //     complete: function(res) {
                    //         if (currentVoiceObj.indexOf("ab_play_")>-1) {
                    //             $('#'+currentVoiceObj+'>em').text("免费收听");
                    //             clearInterval(waveTime);
                    //             $(".appui_qanda-voice-wait").hide();
                    //             $(".appui_qanda-voice-wave").show();
                    //             $('.play-audio').removeClass('bg-green').addClass('bg-blue');
                    //             $('.play-audio i').removeClass('bg-green').addClass('bg-blue');
                    //         }
                    //     }
                    // });
                    wx.onVoicePlayEnd({
