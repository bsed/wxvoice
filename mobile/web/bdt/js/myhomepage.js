...
function doUpdateListItem(groups) {
    ...
    //var location = "";
    //location = indexID * 25 + "%";
    //增加了判断，如果是普通用户不显示我答,为了不影响之前的结构，因此在前面加了一个隐藏的标签达到最小的改动 dzb
    if(groups.length==2){
...
function gotoFound_friends_message_detailHtml(acticleID){
  // window.location = "found_friends_message_detail.html?id="+acticleID;
    window.location.href = "found_friends_message_detail.html?id="+acticleID+"&nid="+1;
}
