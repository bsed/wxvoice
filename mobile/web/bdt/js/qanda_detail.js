...
function showHowToShare(){
	$("#mask_mod").show();
	$("#finishConform2").show();
	// $.each($("#finishConform2 div a"), function(index, val) {
	...
	// 	});
	// });
	$("#finishConform2 .finishTouch1").on('click',function(event) {
			event.preventDefault();
			$("#mask_mod").fadeOut();
			$("#finishConform2").fadeOut();
	 	dataLoading("您的专属海报生成中");
			canvas_init("1");
			createStatWithParamlog(window.location.href,"/showHowToShare.html","1",window.location.search);
	});
...
//		tips =tips +"收听";
//        answerType ='<div class="appui-qanda-answerstyle voice '+statusListen+'" id="a_play_'+index+'_'+cell.id+'" onclick = "playAudioClickFunction('+cell.id+',1,1,\'a_play_'+index+'_'+cell.id+'\');">'+
//						'<i></i>'+
...
//		tips =tips +"阅读";
//        answerType = 	'<div class="appui-qanda-answerstyle pictext '+statusListen+'" id="a_play_'+index+'_'+cell.id+'" onclick = "playAudioClickFunction('+cell.id+',1,1,\'a_play_'+index+'_'+cell.id+'\');">'+
//							'<i></i>'+
...
//    }
//    var expertinfoStr = "";
//    if (cell.answerUser.lable!="") {
//    	expertinfoStr = '<p>'+cell.answerUser.lable.replace(/,/g,"、") +'</p>';
//    };				
//	doStr =		'<div class="appui-qanda-module recommend" onclick = "gotoQADetail('+cell.id+','+index+',\''+typeId+'\')">'+
//					//<!--问题信息-->
//					'<div class="appui-qanda-question">'+
//						picsStr+
//						cell.content+
//					'</div>'+
...
function backFunction() {
	createStatWithParamlog(window.location.href,"click","0",window.location.search);
    //typeId 3表示是刚刚支付返回的界面；1表示我问列表  myanswer.html#1 10我的答案10 11 myhomepage.html#1, 12我问界面 13我听界面 2拒绝回答, 3回答问题成功, 4重新回答原问题，
    // 4 index页面 5 qanda页面 6 user_page 页面;7 quick_response 页面
        if(typeId == "notice"){
            window.location.href = "mynotice.html";
                }else if (typeId=="notice1") { 
                    window.location.href = "mynotice.html#1";
                }else if (typeId=="wxnotice"||typeId==0) { 
                    window.location.href = "index.html";
                }else if (typeId==1) {
        window.location.href = "myanswer.html#0";
                }else if (typeId==10){
                    window.location.href = "myanswer.html#0";
                }else if (typeId==11){
        window.location.href = "myhomepage.html#0";
                }else if (typeId==12) {
        window.location.href = "myhomepage.html#1";
                }else if (typeId==13) {
        window.location.href = "myhomepage.html#2";
                }else if (typeId==20){
                    window.location.href = "qanda.html#0";
                }else if (typeId==21) {
                    window.location.href = "qanda.html#1";
                }else if (typeId==3){
        window.location.href = "myhomepage.html#1";
    }else if (typeId==4){
                var listId = readClientSession("qaLable");
		if(isUndefined(listId)||listId==null||listId==""){
        window.location.href = "index.html";
		}
		else{
        ...
        window.location.href = "quick_response.html";
    }else if (typeId==30){
        var listId = readClientSession("qaLable");
		if(isUndefined(listId)||listId==null||listId==""){
			window.location.href = "index.html";
        	...
        	window.location.href = "qa_label_list.html?listId="+listId;
    	}
    }else{
...
function requestCommentList(){
    ...
    $.ajax({
        ...
        dataType:"json",
        async: true,
        data:{"page":currentPage,"pernum":500,"id":qaId,"contentType":2},
        success:function(result){
            clearToastDialog();
            if(result.result=="success"){
                if (result.data.list.length>0) {
                    totalPage = result.data.page.pages;
                    configCommentUI(result.data.list);
                }else{
...
function configCommentUI(groups){
   ...
   if (totalPage != currentPage) {
        ...
        //     flag = 0;
        // };
        // $('.comment-list-con').append('<a onclick="downloadMoreData();" id="loadMore" class="appui_loadmore fs32 fc-greyabc">拼命加载中<i class="loadmore"></i></a>');
        // $('.comment-list-con').append('<a onclick="downloadMoreData();" class="appui_loadmore bc-grey  fs32 fc-greyabc">加载更多</a>');
   }else{
