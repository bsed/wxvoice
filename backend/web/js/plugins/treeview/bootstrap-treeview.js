	...
	Tree.prototype = {
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				//添加选项
				treeItem.append(self._template.eidt);
				treeItem.append(self._template.add);
                var tags = node.tags;
				...
				if (self.options.showTags && node.tags) {
					...
					$.each(node.tags, function addTag(id, tag) {
						var del = '<a href="javascript:;" class="del" data-id="'+tag+'"><span style="float:right;margin-right:10px;">删除</span></a>';
						treeItem.append(del);
						var edit = '<a href="index.php?r=term/edit&id='+tag+'"><span style="float:right;margin-right:10px;">编辑</span></a>';
						treeItem.append(edit);
						var add ='<a href="index.php?r=term/add&id='+tag+'"><span style="float:right;margin-right:10px;">添加子栏目</span></a>';
						treeItem.append(add);
					});
				}
